...
const char *password = "joham1015morales";
...
int time_between_samples = 10000; // Tiempo entre muestras en milisegundos
...
auto cmp = [](const double a, const double b) { return a < b; }; // Comparador para ordenar de menor a mayor
...
String encryptAES128CBC(const String &plainText) {
    ...
    mbedtls_aes_crypt_cbc(&aes, MBEDTLS_AES_ENCRYPT, len, iv,
                          (const unsigned char *) padded.c_str(), output);
...
String generateSHA256(const String &data) {
    ...
    for (int i = 0; i < 32; i++) {
        char buffer[3];
        sprintf(buffer, "%02x", shaResult[i]);
        hashString += buffer;
    }
    return hashString;
}
